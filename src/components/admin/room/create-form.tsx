		...
		<form action={formAction}>
			<div className="grid md:grid-cols-12 gap-5">
				<div className="col-span-12 md:col-span-8 bg-white p-4">
					<div className="mb-4">
						<input type="text" name="name" placeholder="Room name..." className="py-2 px-4 rounded-sm border border-gray-400 w-full" />
						<div className="" aria-live="polite" aria-atomic="true">
							<span className="text-sm text-red-500 mt-2">{state?.error?.name}</span>
						</div>
					</div>
					<div className="mb-4">
						<textarea name="description" rows={8} placeholder="Description" className="py-2 px-4 rounded-sm border border-gray-400 w-full" />
						<div className="" aria-live="polite" aria-atomic="true">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
							{amenities.map((amenity) => (
								<div className="flex items-center mb-4" key={amenity.id}>
									<input type="checkbox" name="amenities" defaultValue={amenity.id} className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded" />
				...
				<div className="col-span-12 md:col-span-4 bg-white p-4">
					...
					<div className="mb-4">
						<input type="text" name="price" placeholder="Price..." className="py-2 px-4 rounded-sm border border-gray-400 w-full" />
						<div className="" aria-live="polite" aria-atomic="true">
							<span className="text-sm text-red-500 mt-2">{state?.error?.price}</span>
						</div>
					</div>
					<div className="mb-4">
						<input type="text" name="capacity" placeholder="Capacity..." className="py-2 px-4 rounded-sm border border-gray-400 w-full" />
						<div className="" aria-live="polite" aria-atomic="true">
					...
					<button disabled={isPending} className={clsx("bg-orange-400 text-white w-full hover:bg-orange-500 py-2.5 px-6 md:px-10 text-lg font-semibold cursor-pointer", isPending && "opacity-50 cursor-progress")} type="submit">{isPending ? "Saving..." : "Save"}</button>
				</div>
			</div>
		</form>
	)
