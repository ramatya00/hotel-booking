...
export default async function PaymentSuccess({ searchParams }: { searchParams: Promise<{ transaction_status: string }> }) {
	const paymentStatus = (await searchParams).transaction_status;
	if (paymentStatus === "pending") redirect("/payment/pending");
	if (paymentStatus === "failed") redirect("/payment/failure");
